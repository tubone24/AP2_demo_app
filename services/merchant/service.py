...
from common.models import A2AMessage, Signature
from common.database import DatabaseManager, ProductCRUD, MandateCRUD
from common.crypto import SignatureManager, KeyManager
from common.logger import get_logger, log_a2a_message, log_crypto_operation
...
class MerchantService(BaseAgent):
    ...
    def register_endpoints(self):
        ...
        async def get_did_document():
            ...
            did_doc_file = did_docs_dir / "merchant_did.json"
        ...
        async def sign_cart_mandate(sign_request: Dict[str, Any]):
            ...
            try:
                ...
                if self.auto_sign_mode:
                    # ===== 自動署名モード =====
                    # AP2完全準拠：merchant_authorization JWT（のみ）を生成
        ...
        async def get_transactions(status: str = None, limit: int = 100):
            ...
            try:
                async with self.db_manager.get_session() as session:
                    from common.database import TransactionCRUD
    ...
    def _generate_merchant_authorization_jwt(
        ...
        """
        AP2仕様準拠のmerchant_authorization JWTを生成（共通実装を使用）
        ...
        """
        try:
            # AP2完全準拠：CartMandate全体（署名フィールドを除く）をハッシュ化
            # merchant_authorizationはまだ存在しないため、自動的に除外される
            from common.user_authorization import compute_mandate_hash
            from common.jwt_utils import MerchantAuthorizationJWT

            # CartMandateのハッシュを計算（署名フィールドを自動除外）
            ...
            # JWT生成（cart_hashを直接渡す）
            jwt_token = jwt_generator.generate_with_hash(
                merchant_id=merchant_id,
                cart_hash=cart_hash,
                audience="did:ap2:agent:payment_processor",
                expiration_minutes=60,  # 1時間
                algorithm="ECDSA",
                key_id=merchant_id  # AP2完全準拠: DIDドキュメントに登録されているmerchant_idを使用
            )
