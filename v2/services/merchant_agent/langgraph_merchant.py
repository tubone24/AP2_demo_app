...
[
  ...
  {{"plan_name": "高品質プラン", "items": [{{"product_id": "商品ID", "quantity": 1}}], "selling_points": ["ポイント1", "ポイント2"]}}
        ...
        try:
            messages = [
                SystemMessage(content="あなたは「むぎぼー」（かわいい豆柴のキャラクター）を販売するショップの商品選びのプロフェッショナルです。ユーザーに最適な商品プランを提案してください。重要視する点として、ユーザーから提示されている金額や要望を満たしている商品を選び、カートに複数点のグッズを入れるようにしてほしいです。"),
                HumanMessage(content=prompt)
            ]
